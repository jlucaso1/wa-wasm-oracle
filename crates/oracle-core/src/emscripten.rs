...
pub fn define(store: &mut Store<HostState>, linker: &mut Linker<HostState>) -> Result<()> {
    ...
    linker.func_wrap(
        ...
        |caller: Caller<'_, HostState>| -> Result<(), wasmtime::Error> {
            ...
            Err(anyhow!("module called abort()"))
        },
    )?;
    ...
    linker.func_wrap(
        ...
        |mut caller: Caller<'_, HostState>,
         ...
         -> Result<i32, wasmtime::Error> {
            ...
            let state = caller.data();
            state.write(*ptr as u32, &vec![0u8; size as usize])?;
            state.write(addr as u32, &(*ptr as u32).to_le_bytes())?;
            state.write(allocated as u32, &1u32.to_le_bytes())?;
            Ok(0)
        },
    )?;
    ...
    // dispatch simply does not happen: the VoIP engine queues its outbound
    // signaling this way — the callback in table slot 436 is what calls
    // `sendSignalingXMPP_js_sync` — so with this returning zero the engine
    // parses an offer, decides to answer, and the answer is never sent.
    //
    // Arguments, read off the call site (`oracle abi D5pLH9sfOOl --index
    ...
    linker.func_wrap(
        ...
        |mut caller: Caller<'_, HostState>,
         ...
         -> Result<f64, wasmtime::Error> {
            crate::state::sync_memory(&mut caller);
            let callee = table_entry(&mut caller, index as u64)?;
            ...
            for (position, param) in params.iter().enumerate() {
                ...
                call_args.push(match param {
                    ...
                    other => return Err(anyhow!("proxied call takes an unsupported {other:?}")),
                });
            }
    ...
    linker.func_wrap(
        ...
        |caller: Caller<'_, HostState>, code: i32| -> Result<(), wasmtime::Error> {
            ...
            Err(anyhow!("module called exit({code})"))
        },
    )?;
        ...
        let func =
            crate::host::host_func(&mut *store, ty.clone(), move |caller, params, results| {
                ...
                let index = match index {
                    ...
                    other => return Err(anyhow!("{name} index is not i32: {other:?}")),
                };

                let callee = table_entry(caller, index)?;
                ...
                match callee.call(&mut *caller, &args, results) {
                    ...
                    Err(error) => {
                        ...
                        caller.data().log(format!("caught in {name}: {error}"));
                        Ok(())
                    }
                }
            });
        ...
        let func = match import.name() {
            ...
            "_tzset_js" => crate::host::host_func(
                ...
                |caller: &mut Caller<'_, HostState>, params, _results| {
                    ...
                    let state = caller.data();
                    state.write(at(0), &0i32.to_le_bytes())?;
                    state.write(at(1), &0i32.to_le_bytes())?;
                    state.write(at(2), b"UTC\0")?;
                    state.write(at(3), b"UTC\0")?;
                    Ok(())
                },
            ),
