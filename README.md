...
```console
$ oracle call D5pLH9sfOOl getWebP2PVirtualIpv4
Str("192.0.2.1")

$ oracle embind D5pLH9sfOOl
...
```console
$ oracle call D5pLH9sfOOl initVoipStack 15550002222@s.whatsapp.net 0 '{}'
Int(120011)                                  # PJSIP denied a thread

$ oracle call D5pLH9sfOOl initVoipStack 15550002222@s.whatsapp.net 0 '{}' --threads --log
Int(0)
...
| --- | --- | --- | --- |
| `D5pLH9sfOOl` | 9.3 MiB | **VoIP engine** — emscripten + embind + pthreads | 206 functions callable |
...
requires — and draining that queue is how the VoIP engine's outbound signaling
leaves, since the callback in table slot 436 is what calls
`sendSignalingXMPP_js_sync` and nothing calls it directly.
...
crates/oracle-cli the `oracle` binary
```
...
`VOIP_STATUS.md` is the place to start: where an outgoing call currently stops,
